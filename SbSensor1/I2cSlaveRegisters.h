...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool ValidRegister(int Index, int InSize)
{
   if (Index > 255) {
      // Size coded into definition / index value => check
      int Size = Index >> 8;
      if (Size != InSize) {
         printf("Register size error (%d, %d)\n", Index, InSize);
         return false;
      }
   }
   ...
   if ((Index + InSize - 1) < REG_MAP_SIZE) return true;

   printf("Register # out of range (%d, %d, %d)\n", Index, InSize, REG_MAP_SIZE);
   return false;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void RegisterSet(int i, int v)
{
   int Size = i >> 8;

   if (!ValidRegister(i, Size)) return;

   if (Size == 1) RegisterSetByte(i, v);
   if (Size == 2) RegisterSetWord(i, v);
}
...
void requestEvent()
{
  ...
  I2cRegister[R_GENERIC_ID] = IDENTIFICATION; // ID register
